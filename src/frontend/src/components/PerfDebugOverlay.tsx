...
export default function PerfDebugOverlay() {
  ...
  const [markers, setMarkers] = useState<Array<{ name: string; elapsed: number }>>([]);
  ...
  return (
    <div className="fixed bottom-4 left-4 bg-card border border-border rounded-lg shadow-lg p-4 max-w-sm z-50 text-xs font-mono">
      <div className="font-bold mb-2 text-foreground">Performance Diagnostics</div>
      <div className="space-y-1 text-muted-foreground">
        {markers.map((marker, idx) => (
          <div key={idx} className="flex justify-between gap-4">
            <span>{marker.name}:</span>
            <span className="text-foreground font-semibold">{marker.elapsed.toFixed(0)}ms</span>
          </div>
        ))}
        {markers.length > 0 && (
          <div className="flex justify-between gap-4 pt-2 border-t border-border mt-2">
            <span className="font-bold">Total:</span>
            <span className="text-foreground font-bold">{perfDiagnostics.getTotalTime().toFixed(0)}ms</span>
          </div>
        )}
      </div>
    </div>
