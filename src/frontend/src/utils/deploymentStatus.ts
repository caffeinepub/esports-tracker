...
export function getDeploymentStatus(): DeploymentStatus {
  ...
  const timestamp = import.meta.env.VITE_DEPLOYMENT_TIMESTAMP as string | undefined;
  ...
  const errorMessage = import.meta.env.VITE_DEPLOYMENT_ERROR as string | undefined;
...
export function hasDeploymentInfo(): boolean {
  ...
  return status.status !== 'unknown' || !!status.errorMessage || !!status.logUrl;
}
